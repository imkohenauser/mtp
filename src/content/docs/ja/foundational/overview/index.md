...
**座標ベースのコンパイル手順**で、モデルのトーンや語り口をステアリングします。  
...
このページでは、**基本用語**と、MTP のステアリングを支える二つのレイヤーの簡易仕様をまとめます。  
この二つのレイヤーは、実際の挙動に即して言えば **グリッド上の位置** と **強度・極性** として整理できます。  
内部モデルでは **Space** と **Intensity** と呼びます。  
...
たとえば `power:100`、`red:100`、`J:4` のどれを入力しても、コンパイラは同じ 3 要素に解決し、同じ制約を抽出します。
