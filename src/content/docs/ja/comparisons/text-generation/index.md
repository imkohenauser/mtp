...
詳細な比較用プロンプトに入る前に、同じ文学紹介プロンプトへ MTP 引数を加えたときの変化を確認します。日本語の例では、Claude.ai 上の Sonnet 4.6 でルイス・キャロル『不思議の国のアリス』の紹介文を生成しています。

### 単一ノードの強度

`power` と `surge` は、プロンプトを固定したままノードの強度だけを変えています。文の押し出し、緊張感、速度感の違いを比較しやすくしています。
