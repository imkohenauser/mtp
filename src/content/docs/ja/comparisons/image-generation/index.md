...
> [!NOTE]
> MTP Skill は画像生成に最適化されていません。MTP Skill は入力テキストに適応するため、画像生成で使う場合は追加の工夫が必要です。プロンプトでは、Skill を画像の構図、光、スタイル、フレーミングなどへどう反映するかを明示する必要があります。また、画像生成と Agent Skills の連携状況は、モデルやツールによって異なります。以下のプロンプト記録では、`<agent_instruction>` が、MTP Skill の出力を画像生成タスクへ反映するための補助テキストを表します。

---

## MTP Skill による画像生成の変化

詳細なプロンプト記録に入る前に、2 つの作例で MTP の効き方を確認します。題材と構図を固定し、MTP 引数だけを変えることで、画像上の差分を比較しやすくしています。
