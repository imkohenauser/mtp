...
MTP Skill は Agent Skill として配布されています。通常、Skills は、IDE や CLI などの開発者向けツールから利用します。ただし現時点では、Claude.ai のカスタマイズ設定や Manus.im の Skills 管理画面から、ZIP 圧縮した Skill をアップロードすることで、Web 版や iOS アプリからもエージェント スキルを利用できます。MTP Skill は、MTP リポジトリの `skills/mtp` ディレクトリを ZIP 圧縮してアップロードすることで利用できます。
