...
MTP Playlist では、プレイリストをカードデッキやタロットのスプレッドのように扱います。  
左側が Side A（Start と 9 つのノード）、右側が Side B（9 つのノードと End）で、各曲はその位置で固有の役割を担います。

**Start** と **End** はグリッド上のノードではなく、順序で読む際に追加した仮想の枠（概念上のスロット）です。  
MTP Playlist では、それらを半透明のガラスのように浮くノードとして扱い、割り当てられた曲の特徴に応じて周囲のノードへ寄せます。  
たとえば Start に置いた曲が力強ければ、赤軸の Power 方向へ寄ります。

配置そのものが、プレイリストの意味をつくります。

![MTP Playlist の構造図。左に Side A、右に Side B のノード群。各マスは色分けされた背景の上にレコード状の円で示され、Start と End は半透明のガラス状ノードとして浮いている。](/images/pages/mtp-playlist-reading-songs-as-placement.png)

*`1+9+9+1` の Side A（左）と Side B（右）。*
