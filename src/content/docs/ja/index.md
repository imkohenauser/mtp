...
description: MTP は、グリッド、スライダー、プリセットで LLM の出力を方向づけ、ノード対応の音楽シーケンスにも応用できるフレームワークです。
head:
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      ...
      content: https://mappingtheprompt.com/ogp.png
hero:
  title: Mapping the Prompt は LLM の出力を方向づけるためのカラーグリッド型フレームワーク
  tagline: スライダー、グリッド座標、プリセットを使い、タスクを書き換えずに LLM の出力の形を整えます。
lastUpdated: true
---

![MTP のキービジュアル。左のパネルには Open–Still や Power–Void などの Side A / Side B のノードスライダーが示され、Power は 70 に設定されています。右のパネルには A–S と 1–19 のラベルが付いた 19×19 の column:row カラーグリッドが示されています。](/ogp.png)
...
- [MTP Skill](/ja/skills/mtp/): `/mtp` で利用。スライダー、グリッド座標、プリセットで LLM の出力を方向づけます。
- [MTP Playlist Skill](/ja/skills/mtp-playlist/): `/mtp-playlist` で利用。各曲を MTP ノードに対応づけ、その位置で曲がどう働くかを説明します。
...
ロードマップ上の MTP Interactive UI では、スライダーとグリッドを UI 上で操作できるようにする予定です。  
...
![`/mtp node:intensity` によるスライダー引数の説明図。左は Side A と Side B のノードスライダーを示し、Power が 70 に設定されている。右は Open、Power、Focus、Flow、Close、Surge、Collapse、Fade などの方向を、3D チェビシェフ距離として示している。](/images/pages/mtp-slider-arguments-mtp-node-intensity.png)
*スライダー引数は、ノード名と強度値を組み合わせ、MTP 空間の中心から Side A または Side B の方向へ移動する指定方法です。*

### グリッド

![`/mtp column:row` によるグリッド引数の説明図。左は 3×3 の色グリッドを 19×19 座標上に単純化して示し、J:4、D:10、P:10、J:16 などの例を表示している。右は 19×19 全体の RGBA 値の分布を示している。](/images/pages/mtp-grid-arguments-mtp-column-row.png)
*グリッド引数は、19×19 の MTP 座標平面上の一点を指定し、その位置をノード方向と RGBA の色値に対応させる指定方法です。*
