---
title: Comparisons
description: Compare outputs generated with and without MTP Skill across text and image generation tasks.
head:
  - tag: meta
    attrs:
      property: og:image
...
Using the same prompt, this section compares text outputs produced with the `/mtp <args>` command (MTP Skill). You can review the behavior of each command and observe differences across MTP nodes (such as Power and Flow).

Results are compared across major models including ChatGPT, Claude, and Gemini.

[Text Generation](/comparisons/text-generation/) →
